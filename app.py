...
import urllib.parse
import time
from openai import OpenAI  # OpenAI v1.0+ client
    ...
    "credentials": {
        "usernames": {
            ...
            "analyst": {
                ...
                "name": "Analyst User",
                "password": "$2b$12$KIXp1pa8kT2P/QN1jJ1QBu6s9K2fY0jN3VqO/0oV5P2X4zX1X6X2"  # 'analyst123'
            }
        }
    },
    "cookie": {
...
if authentication_status:
    ...
    st.title("🚀 Ultimate Venture Capital Due Diligence Portal")
    ...
    if user.role == 'admin':
        ...
        st.sidebar.info(f"Current Backend: {DB_TYPE.upper()}")

    def ai_analysis(section, inputs):
        """AI-powered risk scoring for a due diligence section."""
        score = 0
        risk_level = "High"
    ...
    # ================================
    # SaaS Metrics Calculation Function
    # ================================
    def calculate_saas_metrics(mrr_start, mrr_end, new_mrr, lost_mrr, expansion_mrr, cac, monthly_burn, gross_margin_percent, num_customers):
        """Calculate comprehensive SaaS metrics including ARPA and Cohort Retention."""
        metrics = {}
    ...
    if page == "Overview & Checklists":
        st.header("Ultimate VC Due Diligence Portal – December 2025")
        st.markdown("Complete authenticated, persistent, collaborative VC tool.")

        st.subheader("Master Due Diligence Checklist")
        dd_sections = [
            "Deal Sourcing (AI-Powered)",
            "Financial Due Diligence",
            "Legal Due Diligence",
        ...
        completed_sections = [a.section for a in session_db.query(Analysis.section).filter_by(user_id=user.id).distinct().all()]

        completion_percentage = (len(set(completed_sections) & set(dd_sections)) / len(dd_sections)) * 100
        st.progress(completion_percentage / 100)
        st.write(f"Overall Completion: {completion_percentage:.0f}%")
        ...
        for section in dd_sections:
            ...
            with col1:
                st.write(section)
            with col2:
                status = "Completed" if section in completed_sections else "Pending"
                st.write(status)
            with col3:
        ...
        if st.button("Download Checklist PDF"):
            ...
            pdf.cell(0, 10, "VC Due Diligence Checklist", ln=1, align='C')
            pdf.set_font("Arial", size=12)
            pdf.ln(10)
            pdf.cell(0, 10, f"Prepared by: {name} | Date: {datetime.date.today()}", ln=1)
            pdf.ln(10)
            for section in dd_sections:
                status = "Completed" if section in completed_sections else "Pending"
                pdf.cell(0, 10, f"- {section}: {status}", ln=1)
            buffer = io.BytesIO()
            pdf.output(buffer)
            buffer.seek(0)
            st.download_button("Download PDF", buffer, "vc_checklist.pdf", "application/pdf")

    elif page == "Deal Sourcing (AI-Powered)":
        st.header("🔍 Deal Sourcing")
        st.markdown("Choose sourcing method and upload supporting documents.")
        ...
        if sourcing_method == "Crunchbase Search":
            ...
            st.markdown("Crunchbase API Integration Guide: Register at crunchbase.com/developers, get API key, use endpoints like /organizations for search. Rate limit: 1000 calls/month free. Error codes: 401 unauthorized, 429 rate limit.")
            ...
            if st.button("Search Crunchbase"):
                if not api_key:
                    ...
                    try:
                        ...
                        new_analysis = Analysis(user_id=user.id, section="Deal Sourcing", data=json.dumps(deal_data))
            ...
            st.markdown("**AngelList API Integration Guide**")
            st.markdown("1. Register at wellfound.com/developers. 2. Create app, get Client ID/Secret. 3. Use OAuth 2.0. 4. Endpoint: https://api.wellfound.com/v1/companies/search")
            ...
            st.markdown("**LinkedIn OAuth 2.0 Guide**")
            st.markdown("1. Create app at developer.linkedin.com. 2. Add redirect URI. 3. Request r_organization_lookup permission.")
            ...
            st.markdown("**Google Search Integration Guide**")
            st.markdown("1. Enable Custom Search Engine at programmable-search-engine.google.com. 2. Get Search Engine ID. 3. Get API key from console.cloud.google.com/apis.")
            ...
            st.markdown("Use OpenAI GPT-4o to generate startup ideas, search for trends, or find companies based on criteria.")
            if not openai_client:
                ...
                if st.button("Generate Startup Leads with GPT-4o"):
                    try:
                        ...
                        st.error("OpenAI rate limit exceeded. Wait and try again.")
            ...
            st.markdown("Use Google Gemini to search for startups, market trends, or generate deal ideas.")
                ...
                if st.button("Generate Leads with Gemini"):
                    try:
                        ...
                        if e.response.status_code == 429:
                            st.error("Gemini rate limit exceeded. Wait and try again.")
                        else:
                            st.error(f"HTTP error: {e.response.status_code}")
                    except requests.exceptions.RequestException as e:
                        st.error(f"Network error: {e}")
            ...
            st.markdown("Use xAI Grok to search for startups, market trends, or generate deal ideas.")
            if not grok_api_key:
                ...
                if st.button("Generate Leads with Grok"):
                    try:
                        ...
                        if e.response.status_code == 429:
                            st.error("Grok rate limit exceeded. Wait and try again.")
                        elif e.response.status_code == 401:
                            st.error("Invalid Grok API key.")
                        else:
                            st.error(f"HTTP error: {e.response.status_code}")
                    except requests.exceptions.RequestException as e:
                        st.error(f"Network error: {e}")
            ...
            if submitted:
                ...
                if errors:
                    ...
                    new_analysis = Analysis(user_id=user.id, section="Deal Sourcing", data=json.dumps(manual_data))
                    session_db.add(new_analysis)
                    session_db.commit()
                    st.success("Manual deal saved with uploaded documents.")

    elif page == "Financial Due Diligence":
        st.header("🔍 Financial Due Diligence")
        st.markdown("Enter key SaaS metrics for AI analysis and advanced calculations.")
        ...
        with st.form("financial_form"):
            ...
            submitted = st.form_submit_button("Calculate SaaS Metrics & Run AI Analysis")
        ...
        if submitted:
            ...
            new_analysis = Analysis(user_id=user.id, section="Financial DD", data=json.dumps(save_data))
            session_db.add(new_analysis)
            session_db.commit()
            st.success("Financial analysis and SaaS metrics saved.")

    elif page == "Legal Due Diligence":
        st.header("⚖️ Legal Due Diligence")
        st.markdown("Assess legal risks. Flag any known issues.")
        ...
        if submitted:
            ...
            new_analysis = Analysis(user_id=user.id, section="Legal DD", data=json.dumps(save_data))
            session_db.add(new_analysis)
            session_db.commit()
            st.success("Legal analysis saved.")

    elif page == "Technical Due Diligence":
        st.header("🛠️ Technical Due Diligence")
        st.markdown("Assess technical risks and capabilities. Flag any known issues.")
        ...
        if submitted:
            ...
            save_data = inputs | {"notes": notes, "score": score}
            new_analysis = Analysis(user_id=user.id, section="Technical DD", data=json.dumps(save_data))
            session_db.add(new_analysis)
            session_db.commit()
            st.success("Technical analysis saved.")

    elif page == "Operational Due Diligence":
        st.header("🏢 Operational Due Diligence")
        st.markdown("Assess operational risks and efficiency. Flag any known issues.")
        ...
        if submitted:
            ...
            save_data = inputs | {"notes": notes, "score": score}
            new_analysis = Analysis(user_id=user.id, section="Operational DD", data=json.dumps(save_data))
            session_db.add(new_analysis)
            session_db.commit()
            st.success("Operational analysis saved.")

    elif page == "Market Due Diligence":
        st.header("🌍 Market Due Diligence")
        st.markdown("Assess market risks and attractiveness. Flag any known issues.")
        ...
        if submitted:
            ...
            save_data = inputs | {"notes": notes, "score": score}
            new_analysis = Analysis(user_id=user.id, section="Market DD", data=json.dumps(save_data))
            session_db.add(new_analysis)
            session_db.commit()
            st.success("Market analysis saved.")

    elif page == "Commercial Due Diligence":
        st.header("💼 Commercial Due Diligence")
        st.markdown("Assess commercial viability and go-to-market risks. Flag any known issues.")
        ...
        if submitted:
            ...
            save_data = inputs | {"notes": notes, "score": score}
            new_analysis = Analysis(user_id=user.id, section="Commercial DD", data=json.dumps(save_data))
            session_db.add(new_analysis)
            session_db.commit()
            st.success("Commercial analysis saved.")

    elif page == "Team & Founder Analysis":
        st.header("👥 Team & Founder Analysis")
        st.markdown("Evaluate the founding team and key hires.")
        ...
        if submitted:
            ...
            save_data = inputs | {"notes": notes, "score": score}
            new_analysis = Analysis(user_id=user.id, section="Team Analysis", data=json.dumps(save_data))
            session_db.add(new_analysis)
            session_db.commit()
            st.success("Team analysis saved.")

    elif page == "ESG & Responsible AI Check":
        st.header("🌿 ESG & Responsible AI Check")
        st.markdown("Evaluate environmental, social, governance, and AI ethics factors.")
        ...
        if submitted:
            ...
            save_data = inputs | {"notes": notes, "score": score}
            new_analysis = Analysis(user_id=user.id, section="ESG", data=json.dumps(save_data))
            session_db.add(new_analysis)
            session_db.commit()
            st.success("ESG analysis saved.")

    elif page == "Term Sheet Negotiation":
        ...
        st.markdown("Build a 36-month financial projection.")
        ...
        if st.button("Generate 36-Month Model"):
            ...
            chart = alt.Chart(df.reset_index()).mark_line().encode(x='Month', y='ARR')
            st.altair_chart(chart, use_container_width=True)
            new_analysis = Analysis(user_id=user.id, section="Financial Model", data=df.to_json())
            session_db.add(new_analysis)
            session_db.commit()
            output = io.BytesIO()
            ...
            output.seek(0)
            st.download_button("Download Model", data=output, file_name="financial_model.xlsx")
        ...
        if st.button("Calculate DCF"):
            ...
            st.dataframe(df)
            new_analysis = Analysis(user_id=user.id, section="DCF", data=json.dumps({"ev": ev}))
            session_db.add(new_analysis)
            session_db.commit()

    elif page == "Reverse DCF":
        ...
        if st.button("Calculate Implied Growth"):
            ...
            new_analysis = Analysis(user_id=user.id, section="Reverse DCF", data=json.dumps({"implied_growth": implied_g}))
            session_db.add(new_analysis)
            session_db.commit()

    elif page == "Comparable Analysis":
        st.header("📈 Comparable Company Analysis")
        st.markdown("Enter your startup's metrics and peer data for valuation comparison.")
        ...
        if submitted:
            try:
                ...
                new_analysis = Analysis(user_id=user.id, section="Comparable Analysis", data=json.dumps(save_data))
                session_db.add(new_analysis)
                session_db.commit()
                st.success("Comps analysis saved.")

    elif page == "Market & Competitor Benchmarking":
        ...
        if st.button("Analyze"):
            try:
                ...
                new_analysis = Analysis(user_id=user.id, section="Market Benchmark", data=json.dumps(save_data))
                session_db.add(new_analysis)
                session_db.commit()
            except:
        ...
        new_analysis = Analysis(user_id=user.id, section="Sensitivity", data=pivot.to_json())
        session_db.add(new_analysis)
        session_db.commit()
        ...
        if st.button("Run Simulation"):
            ...
            if evs:
                ...
                st.altair_chart(chart, use_container_width=True)
                new_analysis = Analysis(user_id=user.id, section="Monte Carlo", data=json.dumps({"mean": mean_ev}))
                session_db.add(new_analysis)
                session_db.commit()
        ...
        if st.button("Generate Scenarios"):
            ...
            st.altair_chart(chart, use_container_width=True)
            new_analysis = Analysis(user_id=user.id, section="Scenario Planning", data=json.dumps({s: scenario_dfs[s].to_dict() for s in scenarios}))
            session_db.add(new_analysis)
            session_db.commit()

    elif page == "Generate Report":
        st.header("📑 Generate Comprehensive Due Diligence Report")
        company_name = st.text_input("Company Name")
        if st.button("Generate PDF Report"):
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font("Arial", 'B', 16)
            pdf.cell(0, 10, f"Due Diligence Report - {company_name}", ln=1, align='C')
            pdf.set_font("Arial", size=12)
            pdf.ln(10)
            pdf.cell(0, 10, f"Prepared by: {name} | Date: {datetime.date.today()}", ln=1)
            pdf.ln(10)
            pdf.cell(0, 10, "Summary of Analyses:", ln=1)
            analyses = session_db.query(Analysis).filter_by(user_id=user.id).order_by(Analysis.timestamp.desc()).all()
            for a in analyses:
            ...
            pdf.output(buffer)
            buffer.seek(0)
            st.download_button("Download Report", buffer, f"DD_Report_{company_name}.pdf")
